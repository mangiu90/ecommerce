        ...
        <aside>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($category->subcategories as $subcategory)
                    <li class="py-2 text-sm">
                        <a wire:click="setSubcategoria('{{ $subcategory->name }}')" class="cursor-pointer hover:text-orange-500 capitalize {{ $subcategory->name == $subcategoria ? 'text-orange-500 font-semibold' : '' }}">{{ $subcategory->name }}</a>
                    </li>
                @endforeach
            </ul>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($category->brands as $brand)
                    <li class="py-2 text-sm">
                        <a wire:click="setMarca('{{ $brand->name }}')" class="cursor-pointer hover:text-orange-500 capitalize {{ $brand->name == $marca ? 'text-orange-500 font-semibold' : '' }}">{{ $brand->name }}</a>
                    </li>
                @endforeach
            </ul>

            <x-jet-button wire:click="limpiar" class="mt-4">
                Eliminar filtros
            </x-jet-button>
        </aside>
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <div class="flex items-center">
                                            <ul class="flex text-sm">
                                                <li>
                                                    <i class="fas fa-start text-yellow-400 mr-1"></i>
                                                </li>
                                                <li>
                                                    <i class="fas fa-start text-yellow-400 mr-1"></i>
                                                </li>
